/**
 * Seed mock data into Neo4j.
 *
 * Creates 60 events spread across today through day+14 (many overlapping at
 * the same day/time so the Event Suggestions page has interesting conflicts to rank).
 *
 * Saurabh Khire hosts exactly 2 events (checked in only there):
 *   • Bay Area Founders Mixer — mixer / icebreaker room (60 checked in)
 *   • Tech Startup 1-on-1s — 1:1 schedule (60 checked in)
 * All other events: 10 checked-in guests, no host. Bulk Neo4j writes use UNWIND + one write transaction per chunk.
 *
 * Run:  npm run db:mock   (from repo root or server/)
 */
...
const { runQuery, runWrite, verifyConnection, closeDriver } = require('../src/db/neo4j');
const neo4jDal = require('../src/db/neo4j-dal');
...
const ATTENDEES_EACH = 60;
const OTHER_EVENT_ATTENDEES = 10;
const NEO_SEED_CHUNK = 12;
...
const PAST_EVENTS_POOL = [
  [
    { eventName: 'YC Startup School Mixer', eventDate: daysAgo(45), eventType: 'mixer', peopleMet: 8, connectionsMade: 3, highlights: 'Met two potential co-investors; strong founder conversations.', rating: 5 },
    { eventName: 'SaaStr Annual Networking Night', eventDate: daysAgo(90), eventType: 'mixer', peopleMet: 12, connectionsMade: 5, highlights: 'Partnership lead from a single intro.', rating: 5 },
  ],
  [
    { eventName: 'Product Hunt Makers Festival', eventDate: daysAgo(60), eventType: 'mixer', peopleMet: 15, connectionsMade: 6, highlights: 'Launch traffic and 3 investor intros same week.', rating: 5 },
    { eventName: 'First Round Fast Forward', eventDate: daysAgo(30), eventType: 'personal', peopleMet: 6, connectionsMade: 4, highlights: 'Tactical advice on scaling ARR.', rating: 5 },
  ],
  [
    { eventName: 'Sequoia Arc Cohort Mixer', eventDate: daysAgo(120), eventType: 'mixer', peopleMet: 20, connectionsMade: 8, highlights: 'Portfolio intros turned into pilots.', rating: 5 },
    { eventName: 'Founder Summit NYC', eventDate: daysAgo(75), eventType: 'normal', peopleMet: 10, connectionsMade: 4, highlights: 'Enterprise sales panel → 2 lead investors.', rating: 4 },
  ],
  [
    { eventName: 'Growth Hackers Conference', eventDate: daysAgo(55), eventType: 'normal', peopleMet: 25, connectionsMade: 7, highlights: 'Talk on viral loops → consulting inquiries.', rating: 5 },
    { eventName: 'B2B SaaS Founders Meetup', eventDate: daysAgo(25), eventType: 'mixer', peopleMet: 9, connectionsMade: 3, highlights: 'PLG vs sales-led debate with peers.', rating: 4 },
  ],
  [
    { eventName: 'Design Matters SF', eventDate: daysAgo(40), eventType: 'mixer', peopleMet: 11, connectionsMade: 5, highlights: 'Contract work from Figma/Notion alumni.', rating: 5 },
    { eventName: 'UX Research Summit', eventDate: daysAgo(100), eventType: 'normal', peopleMet: 8, connectionsMade: 3, highlights: 'Case study feature from design system talk.', rating: 4 },
  ],
];
...
async function ensureSkills() {
  const skills = SKILL_POOL.map(id => ({
    id,
  ...
  await runQuery(
    `UNWIND $skills AS s
     ...
     SET sk.name = coalesce(sk.name, s.name), sk.category = coalesce(sk.category, s.cat)`,
    { skills }
  );
  console.log(`✓ ${SKILL_POOL.length} skills ensured`);
}

async function createEvent(cfg) {
  const id = uuidv4();
  const et = endTime(cfg.startTime, cfg.durationMins);
  await runQuery(
    `CREATE (e:Event {
    ...
    })`,
    {
      ...
      durationMins: Number(cfg.durationMins),
      roundMins: Number(cfg.roundMins),
      groupSizeMin: Number(cfg.groupSizeMin),
      groupSizeMax: Number(cfg.groupSizeMax),
      hostId: cfg.hostId || null,
      description: cfg.description || '',
      location: cfg.location || '',
      blasts: Array.isArray(cfg.blasts) ? cfg.blasts : [],
    }
  );
  return id;
}
...
async function seedSaurabhKhire() {
  ...
  await runQuery(
    ...
    })`,
    {
      ...
      eventIntent: 'Hosting curated networking events for founders, investors, and builders.',
      email: 'saurabhskhire@gmail.com',
      summary: 'Founder and engineer building AI-powered networking tools for professionals. Passionate about combining technology with meaningful human connections.',
  ...
  for (const skillId of ['sk_python', 'sk_js', 'sk_react', 'sk_node', 'sk_ml', 'sk_prodstrat']) {
    await runQuery(
      `MATCH (p:Person {id: $personId}) MATCH (s:Skill {id: $skillId})
       MERGE (p)-[:HAS_SKILL {level: 'senior'}]->(s)`,
      { personId, skillId }
    );
  }
  for (const skillId of ['sk_fundraise', 'sk_gtm', 'sk_bizdev']) {
    await runQuery(
      `MATCH (p:Person {id: $personId}) MATCH (s:Skill {id: $skillId})
       MERGE (p)-[:SEEKS_SKILL {urgency: 'high'}]->(s)`,
      { personId, skillId }
    );
  }

  for (const pev of [
    { eventName: 'HackWithBay 2.0', eventDate: daysAgo(30), eventType: 'normal', peopleMet: 15, connectionsMade: 8, highlights: 'Won first place with VentureGraph.', rating: 5 },
    { eventName: 'Bay Area Startup Mixer', eventDate: daysAgo(60), eventType: 'mixer', peopleMet: 12, connectionsMade: 5, highlights: 'Three investor intro meetings scheduled.', rating: 5 },
    { eventName: 'YC Alumni Meetup', eventDate: daysAgo(90), eventType: 'mixer', peopleMet: 10, connectionsMade: 4, highlights: 'Found two potential advisors.', rating: 4 },
  ]) {
    await neo4jDal.addPastEventHistory(personId, pev);
  }
  await neo4jDal.saveProfile(personId, {
    email: 'saurabhskhire@gmail.com',
    headline: 'Founder at SmartNetworking | AI + Networking | Building tools that make connections matter',
    description: 'Founder and engineer building AI-powered networking tools for professionals.',
    linkedinUrl: 'https://linkedin.com/in/saurabhkhire',
    previousCompanies: ['SmartNetworking', 'HackWithBay', 'Tech Startup'],
    certifications: ['AWS Solutions Architect', 'Full Stack Developer Certification', 'AI Product Management'],
    projects: [
      { name: 'SmartNetworking', description: 'AI-powered event networking platform with intelligent matching.', url: '' },
      { name: 'VentureGraph', description: 'Graph-based startup connection platform for HackWithBay 2.0.', url: '' },
    ],
    summary: 'Founder building AI-powered networking tools.',
  });

  console.log('✓ Saurabh Khire seeded as host profile');
  return personId;
}

function buildPersonPayload(idx) {
  const personId = uuidv4();
  const goalId = uuidv4();
  const name = pickName(idx);
  const email = `${name.toLowerCase().replace(/\s+/g, '.')}.${idx}@techfounder.dev`;
  ...
  const linkedinUrl = `https://linkedin.com/in/${name.toLowerCase().replace(/\s+/g, '-')}-${idx}`;
  ...
  return {
    personId, goalId, idx, name, email, roles, seeksRoles, purpose, companyStage, companyName,
    have, seek, eventIntent, summary, headline, linkedinUrl, workExperience, certifications, projects,
  };
}

async function seedAttendeesNeo(eventId, count, globalOffset) {
  for (let start = 0; start < count; start += NEO_SEED_CHUNK) {
    const slice = Math.min(NEO_SEED_CHUNK, count - start);
    const chunk = [];
    for (let j = 0; j < slice; j++) {
      chunk.push(buildPersonPayload(globalOffset + start + j));
    }
    ...
    const rows = chunk.map((c) => ({
      ...
      purpose: c.purpose,
      seeksRoles: c.seeksRoles,
      eventIntent: c.eventIntent,
      email: c.email,
      summary: c.summary,
      headline: c.headline,
      linkedinUrl: c.linkedinUrl,
      workExperienceJson: JSON.stringify(c.workExperience),
      certifications: c.certifications,
      projectsJson: JSON.stringify(c.projects),
    }));

    const havePairs = [];
    const seekPairs = [];
    for (const c of chunk) {
      for (const skillId of c.have) havePairs.push({ personId: c.personId, skillId });
      for (const skillId of c.seek) seekPairs.push({ personId: c.personId, skillId });
    }

    const pastRows = [];
    for (const c of chunk) {
      for (const pev of PAST_EVENTS_POOL[c.idx % PAST_EVENTS_POOL.length]) {
        pastRows.push({
          personId: c.personId,
          peId: uuidv4(),
          eventName: pev.eventName,
          eventDate: pev.eventDate,
          eventType: pev.eventType,
          peopleMet: pev.peopleMet,
          connectionsMade: pev.connectionsMade,
          highlights: pev.highlights,
          rating: pev.rating,
        });
      }
    }

    const profRows = chunk.map((c) => ({
      personId: c.personId,
      email: c.email,
      headline: c.headline,
      description: c.summary,
      linkedinUrl: c.linkedinUrl,
      previousCompanies: JSON.stringify(c.workExperience.map((w) => w.company)),
    ...
    await runWrite(async (tx) => {
      await tx.run(
        `UNWIND $rows AS row
         CREATE (p:Person {
           ...
           workExperience: row.workExperienceJson,
      ...
      if (havePairs.length) {
        await tx.run(
          `UNWIND $pairs AS pair
           MATCH (p:Person {id: pair.personId})
           MATCH (s:Skill {id: pair.skillId})
           MERGE (p)-[:HAS_SKILL {level: 'mid'}]->(s)`,
          { pairs: havePairs }
        );
      }
      if (seekPairs.length) {
        await tx.run(
          `UNWIND $pairs AS pair
           MATCH (p:Person {id: pair.personId})
           MATCH (s:Skill {id: pair.skillId})
           MERGE (p)-[:SEEKS_SKILL {urgency: 'medium'}]->(s)`,
          { pairs: seekPairs }
        );
      }
      ...
      await tx.run(
        `UNWIND $rows AS row
         ...
         CREATE (g:Goal {id: row.goalId, personId: row.personId, eventId: $eventId, purposeTag: row.purpose})
         MERGE (p)-[:HAS_GOAL]->(g)
         MERGE (p)-[a:ATTENDED]->(e)
         SET a.checkedInAt = datetime()`,
        { eventId, rows }
      );
      ...
      await tx.run(
        `UNWIND $profs AS pr
         MERGE (prof:Profile {personId: pr.personId})
         SET prof.email = pr.email,
             prof.headline = pr.headline,
             prof.description = pr.description,
             prof.linkedinUrl = pr.linkedinUrl,
             prof.previousCompanies = pr.previousCompanies,
             prof.certifications = pr.certifications,
...
async function main() {
  ...
  for (let ei = 0; ei < EVENTS.length; ei++) {
    ...
    const eventId = await createEvent({
      ...
      blasts: (cfg.blasts && cfg.blasts.length) ? cfg.blasts : DEFAULT_BLASTS,
    ...
    if (saurabhHosts) {
      await runQuery(
        `MATCH (p:Person {id: $pid}) MATCH (e:Event {id: $eid})
         MERGE (p)-[a:ATTENDED]->(e) SET a.checkedInAt = datetime()`,
        { pid: saurabhId, eid: eventId }
      );
    }
    const tag = saurabhHosts
      ? ` <- Saurabh host + ${ATTENDEES_EACH} checked in`
      : ` <- ${OTHER_EVENT_ATTENDEES} checked in`;
    console.log(`  [${cfg.type.padEnd(8)}] ${cfg.name} (${cfg.date} ${cfg.startTime})${tag}`);

    if (saurabhHosts) {
      process.stdout.write('  ');
      await seedAttendeesNeo(eventId, ATTENDEES_EACH, fullRoomOffset);
      fullRoomOffset += ATTENDEES_EACH;
      fullRoomEventCount++;
    } else {
      const fillerOffset = 20_000 + ei * 137 + (cfg.name.length % 41);
      process.stdout.write('  ');
      await seedAttendeesNeo(eventId, OTHER_EVENT_ATTENDEES, fillerOffset);
    }
  }

  // Verification
  const evN = toNum((await runQuery('MATCH (e:Event) RETURN count(e) AS n'))[0]?.get('n'));
  const peN = toNum((await runQuery('MATCH (p:Person) RETURN count(p) AS n'))[0]?.get('n'));
  console.log(`\n✓ ${evN} events  |  ${peN} people (${fullRoomEventCount} host events × ${ATTENDEES_EACH} + others × ${OTHER_EVENT_ATTENDEES})`);
  console.log(`  Saurabh hosts: ${[...SAURABH_HOST_EVENTS].join(' · ')}`);
  console.log(`  Open the Organizer dashboard and pick one of those events to compute scores.`);
  await closeDriver();
}
